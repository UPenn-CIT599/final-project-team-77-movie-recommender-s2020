...
public class Recommender {
	...
	*/
	public ArrayList<Movie> CompareInput(String genre, int year, double rating, String actors, String director) {
		...
		int actorsAndDirectors = 0;
		for(int i = 1; i < movieData.size(); i++) {
			//Ensuring there is some actor/director in the movie data
			if(movieData.get(i).getActors().contains(actors) || movieData.get(i).getDirector().contains(director)) {
				actorsAndDirectors++;
			}
		}
		for(int i = 1; i < movieData.size(); i++) {
			...
			//Else if statement in case there are no actors or directors in the data base that match the input
			else if((movieData.get(i).getGenre().contains(genre) || genre.isEmpty()) && movieData.get(i).getMovieRating() <= rating)
			{
				//Ensuring the year range is respected
				if(year == 1 && movieData.get(i).getMovieYear() < 1930) {
					finalRecommendations.add(movieData.get(i));
